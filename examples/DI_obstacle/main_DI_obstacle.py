...
if __name__ == '__main__':
    ...
    circ_center, circ_radius = [5.0, 0.0], 1.0
    h04 = lambda x: (x[:, 0].unsqueeze(-1) - circ_center[0]) ** 2 + (x[:, 1].unsqueeze(-1) - circ_center[1]) ** 2 - circ_radius
    ...
    def plot_env(ax):
        ...
        ax.add_patch(
            plt.Circle((circ_center[0], circ_center[1]), circ_radius, edgecolor='k', facecolor='none', linewidth=3))
    ...
    c20 = Composite_Set('AND', input_sets=[c10, c04], param=composition_param)

    diff_net = Positive_MLP([x_dim, 500, 300, 200, 100, 1], activation='silu', positivity_transform='square')
    psi_0 = Barrier_Fcn(c20, diff_net, freeze_upper_bound_net=True)
    ...
    if plot_options['view_cl_simulation']:
        ...
        x_goal = torch.tensor([[2.0, 0.0]]).to(device)
        init_state = torch.tensor([[5.0, 1.0]]).to(device)
