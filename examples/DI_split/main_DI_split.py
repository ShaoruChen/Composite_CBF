...
from composite_CBF.utils import volume_eval_safe_control_set
...
if __name__ == '__main__':
    ...
    parser.add_argument('--objective', type=str, default='cbvf', choices=['feasibility', 'cbvf'])
    ...
    aug_x_min, aug_x_max = x_min - 1.0, x_max + 1.0

    # unsafe region
    x0_safe_region_1_min = torch.tensor([0.0]).to(device)
    x0_safe_region_1_max = torch.tensor([4.0]).to(device)

    x0_safe_region_2_min = torch.tensor([5.0]).to(device)
    x0_safe_region_2_max = torch.tensor([10.0]).to(device)
    ...
    def plot_env(ax):
        ...
        # plot the circular unsafe set
        unsafe_min = np.array([4.0, -5.0])
        unsafe_max = np.array([5.0, 5.0])
        plot_box(ax, unsafe_min, unsafe_max, color='k', linewidth=3)

    h00 = lambda x: x[:,0].unsqueeze(-1) - x0_safe_region_1_min.item()
    h01 = lambda x: x0_safe_region_1_max.item() - x[:,0].unsqueeze(-1)

    h02 = lambda x: x0_safe_region_2_max.item() - x[:,0].unsqueeze(-1)
    h03 = lambda x: x[:,0].unsqueeze(-1) - x0_safe_region_2_min.item()

    h04 = lambda x: x[:,0].unsqueeze(-1) - x_min[0].item()
    h05 = lambda x: x_max[0].item() - x[:,0].unsqueeze(-1)
    h06 = lambda x: x[:,1].unsqueeze(-1) - x_min[1].item()
    h07 = lambda x: x_max[1].item() - x[:,1].unsqueeze(-1)

    composition_param = 5.0
    ...
    c04 = Composite_Set('INIT',h=h04)
    c05 = Composite_Set('INIT',h=h05)
    c06 = Composite_Set('INIT',h=h06)
    c07 = Composite_Set('INIT',h=h07)

    c10 = Composite_Set('AND', input_sets = [c00, c01], param=composition_param)
    c11 = Composite_Set('AND', input_sets = [c02, c03], param=composition_param)

    c20 = Composite_Set('OR', input_sets = [c10, c11], param=composition_param)
    c21 = Composite_Set('AND', input_sets = [c04, c05, c06, c07], param=composition_param)

    c30 = Composite_Set('AND', input_sets = [c20, c21], param=composition_param)

    diff_net = Positive_MLP([x_dim, 400, 300, 200, 100, 1], activation='silu', positivity_transform='square')

    psi_0 = Barrier_Fcn(c30, diff_net, freeze_upper_bound_net=True)
    ...
    g = agent.g
    relative_degree = 1
    hocbf = HOCBF(psi_0, f, g, relative_degree)

    hocbf = hocbf.to(device)

    xx = torch.rand(4,2).to(device)
    xx.requires_grad_(True)
    psi_vals, Lf_component, Lg_component = hocbf(xx)
    ...
    if args.train:
        # train a HOCBF
        expanded_domain = HyperRectangle(x_min-1.0, x_max+1.0)
        train_samples = expanded_domain.random_sample_from_domain(10000)
    ...
    if plot_options['view_cl_simulation']:
        ...
        x_goal = torch.tensor([[3.0, 0.0]]).to(device)
        init_state = torch.tensor([[9.0, -2.0]]).to(device)
        ...
        # fixed-step integrator: euler, midpoint, rk4, explicit_adams, implicit_adams
        # adaptive-step integrator: dopri5, dopri8, bosh3, fehlberg2, adaptive_heun
        filtered_cl_traj = odeint(dynamics.filtered_cl_dynamics, init_state, interval_ts, method='fehlberg2')
